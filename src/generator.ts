...
function makeEsLintIgnoreRulesString({addNamespacesIgnore}: {addNamespacesIgnore?: boolean} = {}) {
  const ignoreList = [
  ...
  if (addNamespacesIgnore) {
    ignoreList.push('@typescript-eslint/no-namespace');
  }
...
${makeEsLintIgnoreRulesString({addNamespacesIgnore: Boolean(namespace)})}
